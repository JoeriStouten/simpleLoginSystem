...
frame5 = Frame(root, bd=2, relief=RIDGE)
frame5.grid(row=1, column=1)

#let the user know to register first + two entry fields for a username and a password
...
button = Button(frame, command=addToList, text="Add")
button.pack()
...
def login():
    ...
    if loginUsername.get() in userList and loginPassw.get() in passwList:
        ...
        if indexUser == indexPassw:
            ...
            lbl_wrongLogin = Label(frame5, text="Username or password is incorrect")
            lbl_wrongLogin.grid(row=0, column=0)
    else:
        lbl_wrongLogin = Label(frame5, text="Username or password is incorrect")
        lbl_wrongLogin.grid(row=0, column=0)
